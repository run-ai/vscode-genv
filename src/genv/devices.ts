...
export async function attachDevice(eid: string, index: number, overSubscription: boolean): Promise<string> {
  var command = `attach --eid ${eid} --index ${index}`;

  if (overSubscription) {
    command = `${command} --allow-over-subscription`;
  }
